...
class NebConsumer {
 public:
  ...
  void deliver_callback(uint64_t k, volatile const void *m, size_t proc_id) {
    ...
    if (unlikely(delivered_counter == expected)) {
      bt.stop();
...
int main(int argc, char *argv[]) {
  ...
  // dory::set_thread_name(async_event_thread, "ae_poller");
  ...
  dory::neb::sync::NonEquivocatingBroadcast::run_default_sync_control_path(
  ...
  dory::neb::sync::NonEquivocatingBroadcast neb(self_id, process_ids, cb,
                                                deliver_fn);
