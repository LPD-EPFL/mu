...
def parse_file(f, procs):
    ...
    for d in deliv:
        ...
        if k in frm[p]:
            print("WARN: Process {} delivered more than once key {} from {}".format(pid, k, p))
        frm[p][k] = v
...
files = [f for f in listdir(results.TARGET) if isfile(join(results.TARGET, f)) and f.endswith(".out")]
procs = {}
num_procs = len(files)
        ...
        for k in d:
            ...
            for q in procs:
                ...
                if k in procs[q]['d'][o] and procs[q]['d'][o][k] != m:
                        print("ERR: Process {} delivered ({},{}) but process {} delivered ({},{})".format(p, k, m, q, k, procs[q]['d'][o][k]))
                        exit(1)
...
for p in procs:
    ...
    for o in deliv:
        ...
        for k in d:
            m = d[k]
            if m != procs[o]['b'][k]:
                print("ERR: Process {} delivered ({},{}) from {}, but process {} broadcast ({},{})".format(p, k, m, o, o, k, procs[o]['b'][k]))
                exit(1)
    ...
    for k in bcast:
        ...
        for q in procs:
            if not k in procs[q]['d'][p]:
               print("WARN: Process {} broadcast ({},{}), but process {} did not deliver ({},{})".format(p, k, m, q, k, m)) 
               is_valid = False
    if is_valid:
