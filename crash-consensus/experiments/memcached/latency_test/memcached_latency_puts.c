...
void *writer(void *ptr) {
    ...
    for (int i = 0; i < MEDIAN_SAMPLE_SIZE; i++) {
        ...
        int len = sprintf(tmp, "set %s 0 0 %d\r\n%s\r\n",
            key,
            VALUE_SIZE,
            value);
        ...
        // if (i == 0) {
        //     // We need this to warm-up the consensus module of peregrin.
        //     usleep(1000 * 1000 * 2);
        // } else {
        //     usleep(1000 * 50);
        // }
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5) {
        fprintf(stderr, "Provide memcached keysize, valuesize, ip and memcached port as arguments\n");
        exit(EXIT_FAILURE);
    }
