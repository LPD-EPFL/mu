...
// typedef void (*committer_t)(bool leader, uint8_t *buf, size_t len, void
// *ctx);

// // Need an explicit constructor and destructor.
// extern "C" consensus_t new_consensus(int my_id, int *remote_ids, int
// remote_ids_num); extern "C" void free_consensus(consensus_t c);

// extern "C" void consensus_attach_commit_handler(consensus_t c, committer_t f,
// void *committer_ctx); extern "C" ProposeError consensus_propose(consensus_t
// c, uint8_t *buf, size_t len); extern "C" int
// consensus_potential_leader(consensus_t c);
...
// void consensus_attach_commit_handler(consensus_t c, committer_t f, void
// *committer_ctx) { 	auto cons = reinterpret_cast<dory::Consensus*>(c);
// 	cons->commitHandler([f, committer_ctx](bool leader, uint8_t *buf, size_t
// len) { 		f(leader, buf, len, committer_ctx);
// 	});
// }
...
// 	return
// static_cast<ProposeError>(reinterpret_cast<dory::Consensus*>(c)->propose(buf,
// len));
// }