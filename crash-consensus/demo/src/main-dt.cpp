...
int main(int argc, char* argv[]) {
  ...
  std::thread t1(
      [id, remote_ids, times, payload_size, outstanding_req, &barrier] {
        benchmark(id, remote_ids, times, payload_size, outstanding_req, barrier,
                  dory::ThreadBank::B);
      });
  ...
  // std::thread t2([id, remote_ids, times, payload_size, outstanding_req,
  // &barrier]{
  //   benchmark(id, remote_ids, times, payload_size, outstanding_req, barrier,
  //   dory::ThreadBank::B);
  // });
  ...
  // CPU_ZERO(&cpuset);
  // CPU_SET(16, &cpuset);
  // rc = pthread_setaffinity_np(t2.native_handle(), sizeof(cpu_set_t),
  // &cpuset); std::cout << "Thread pinning " << rc;
